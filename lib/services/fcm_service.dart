...
class FcmService {
  ...
  static Future<void> initialize() async {
    try {
      ...
      _messaging.onTokenRefresh.listen((newToken) async {
        ...
        // Mark as unregistered so it re-sends to backend next login
  ...
  static Future<void> _saveToken(String token) async {
    try {
      ...
      debugPrint('⚠️ Failed to save FCM token to Hive: $e');
    }
  }

  // ─────────────────────────────────────────────────────────
  // Get current FCM token (from cache or fresh)
  // ─────────────────────────────────────────────────────────
  ...
  // Called after user logs in (with their JWT)
  // ─────────────────────────────────────────────────────────
  ...
  static void _setupMessageHandlers() {
    ...
    // Terminated: user taps notification while app is killed
  ...
  // ─────────────────────────────────────────────────────────
  // Show a local notification when FCM arrives in foreground
  // (Otherwise system shows it automatically in background)
  // ─────────────────────────────────────────────────────────
